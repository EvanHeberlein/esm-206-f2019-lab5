...
```{r, include=FALSE}
#Read in/clean data and attach packages
...
Use tidyr::uncount() function to convert data from frequency format to case format
```{r}
...
'Site' is our variable of interest
...
#Is it reasonable to compare means?
```

#### Convert the 'date' column to class 'Date'

We'll use the 'lubridate' package to convert to Date format, and to help us easily parse month & year
...
# Number of lobsters observed at each site
...
Use t.test() function to find confidence intervals (for one sample), and perform t-tests to compare means of two samples (to be covered in lectures week 6)
...
t.test(ivee_lobster) #P value represents odds you found your mean from population whose actual mean is 0

```

#### Two-sample t-test to compare means 

Were samples pulled from populations with significanlty different means? Compared against null hypothesis (no)
...
mn_ttest2 <- t.test(size_mm ~ site, data = lobster_mn) #Comparison of size by site
...
Here is the t-statistic: `r mn_ttest2$statistic` (in-line referencing)
